...
solve([], [Target], _, _, Target, _).
solve(X, Board, PList, Empty, Target, Hist) :-
	member(Start, Board),
	member(Jumped, Board),
	member(End, Empty),
	big(Start, Jumped, End, PList),
	jump(Start, Jumped, End),
	(indeCheck(Start, Jumped, End, Hist); lexorder(Start, Jumped, End, Hist)),
	...
	solve(Next, NewBoard, PList, NewEmpty, Target, [[Start, Jumped, End] | Hist]).
